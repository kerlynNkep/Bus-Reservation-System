...
class UserManager(BaseUserManager):
    ...
    All we have to do is override the `create_user` function which we will use to create `User` objects.
    """

    def create_user(self, username, email, phoneNumber, password=None):
        """Create and return a `User` with an email, username, phoneNumber and password."""
        if username is None:
...
class TravelList(models.Model):
    ...
    def __unicode__(self):
        return self.Pickup_Point + "to" + self.Destination


class Bus(models.Model):
    NumberOfSeats = models.CharField(max_length=300)
...
class Payment(models.Model):
    ...
    def __unicode__(self):
        return "payment" + self.payment_status

    def get_absolute_url(self):
        """Return the url to access a particular instance of a reservtion payment """
        return reverse('')
...
class Reservation(models.Model):
    ...
    travelInfo = models.ForeignKey(TravelList, on_delete=models.CASCADE)
